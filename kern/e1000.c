...
e1000_tx_init() 
{
    ...
    for (i = 0; i < NTXDESC; i++) {
        ...
        tx_queue[i].sta |= E1000_TXD_STA_DD;
    }
...
e1000_transmit(void *data, size_t len) 
{
    ...
    if (!(tx_queue[tail].sta & E1000_TXD_STA_DD))
        return -E_TX_FULL;
